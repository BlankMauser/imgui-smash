...
const char *GetShaderSource(const char *path) {
  ...
  SMASH_ASSERT(FsHelper::isFileExist(path), "Failed to Find File!");
...
CompiledData ImguiShaderCompiler::CompileShader(const char *shaderName) {
  ...
  char vshPath[0x40] = {};
  createPath(vshPath, IMGUI_SMASH_SHADER_PATH, shaderName, "_vsh.glsl");
  char fshPath[0x40] = {};
  createPath(fshPath, IMGUI_SMASH_SHADER_PATH, shaderName, "_fsh.glsl");
  ...
  if (glslcDll->GlslcCompile(&initInfo)) {
    ...
    SMASH_ABORT(0, "Failed to Compile supplied shaders. \nVert Path: %s\nFrag Path: %s\n", vshPath, fshPath);
  }
